...
class ResNet(nn.Module):
    def __init__(self, args, backbone='resnet50', backbone_path=None):
        super().__init__()
        ...
        # print(*list(self.feature_extractor))
        ...
        if args.larger_features:
            # stride 2 --> stride 1
            # conv4 still???
            conv3_block1 = self.feature_extractor[-2][0]
...
class SSD300(nn.Module):
    def __init__(self, args, backbone=None):
        super().__init__()
        if backbone is None:
            backbone = ResNet(args, backbone='resnet50')
        self.feature_extractor = backbone

        self.label_num = args.nb_classes  # number of COCO classes
        self._build_additional_features(self.feature_extractor.out_channels)
        self.num_defaults = [2 + 2*len(args.aspect_ratio_0),
                             2 + 2*len(args.aspect_ratio_1),
                             2 + 2*len(args.aspect_ratio_2),
                             2 + 2*len(args.aspect_ratio_3),
                             2 + 2*len(args.aspect_ratio_4),
                             2 + 2*len(args.aspect_ratio_5)]
