...
def get_train_loader(args, local_seed):
    train_annotate = os.path.join(args.data, "train/_annotations.coco.json")
    ...
    train_pipe = COCOPipeline(batch_size=args.batch_size,
        ...
        default_boxes=dboxes300_coco(args),
    ...
    train_loader = DALICOCOIterator(train_pipe, 107 / args.N_gpu)
    return train_loader
...
def get_val_dataset(args):
    ...
    val_annotate = os.path.join(args.data, "valid/_annotations.coco.json")
    val_coco_root = os.path.join(args.data, "valid")
...
def get_test_dataset(args, dataset):
    ...
    test_annotate = os.path.join(dataset, "test/_annotations.coco.json")
    test_coco_root = os.path.join(dataset, "test")

    test_coco = COCODetection(test_coco_root, test_annotate, test_trans)
    return test_coco
