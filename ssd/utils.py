...
class Encoder(object):
    ...
    def encode(self, bboxes_in, labels_in, criteria=0.5, transform=True):
        ...
        idx = torch.arange(0, best_bbox_idx.size(0), dtype=torch.int64, device=self.dboxes.device)
        best_dbox_idx[best_bbox_idx[idx]] = idx

        # filter IoU > 0.5
        masks = best_dbox_ious > criteria
        # print(masks)
        labels_out = torch.zeros(self.nboxes, dtype=torch.long, device=self.dboxes.device)
...
def dboxes300_coco(args):
    ...
    # figsize = 300
    # feat_size = [75, 38, 19, 10, 5, 3]
    # steps = [4, 8, 16, 32, 64, 100]
    # # use the scales here: https://github.com/amdegroot/ssd.pytorch/blob/master/data/config.py
    # scales = [10, 21, 45, 99, 153, 207, 261]
    # aspect_ratios = [[2, 3], [2, 3], [2, 3], [2, 3], [2], [2]]
    # dboxes = DefaultBoxes(figsize, feat_size, steps, scales, aspect_ratios)
    figsize = 300
    feat_size = [75, 38, 19, 10, 5, 3] if args.larger_features else [38, 19, 10, 5, 3, 1]  # TODO add one final layer when larger feature space?
    steps = [4, 8, 16, 32, 64, 100] if args.larger_features else [8, 16, 32, 64, 100, 300]
    # use the scales here: https://github.com/amdegroot/ssd.pytorch/blob/master/data/config.py
    scales = args.scales
    aspect_ratios = [args.aspect_ratio_0, args.aspect_ratio_1, args.aspect_ratio_2, args.aspect_ratio_3, args.aspect_ratio_4, args.aspect_ratio_5]
    dboxes = DefaultBoxes(figsize, feat_size, steps, scales, aspect_ratios)
    return dboxes
